...
const GOOD: FusionConfig = { panel: [STUB, STUB, STUB], synth: STUB };
...
test("fuse fails with no answer when a panel agent fails", async () => {
  const result = await fuse(
    { panel: [STUB, "opencode-go/not-a-real-model", STUB], synth: STUB },
    PROMPT,
  );
...
test("fuse fails with no answer when synthesis fails", async () => {
  const result = await fuse(
    { panel: [STUB, STUB, STUB], synth: "opencode-go/not-a-real-model" },
    PROMPT,
  );
  expect(result.ok).toBe(false);
  expect("answer" in result).toBe(false);
