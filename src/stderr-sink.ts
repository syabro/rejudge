...
/**
 * A progress sink that writes the fusion's activity to **stderr** as a plain append log —
 * the CLI/demo renderer. (The `fusion_agents` tool uses a live in-place block instead; see
 * the extension.) stderr is the right channel here: the fused answer owns stdout, so the log
  ...
  return (event) => {
    switch (event.kind) {
      ...
      case "total": {
        ...
        console.error(`fusion ${word} in ${formatDur(event.durationMs)}`);
        return;
      }
      case "diagnostic":
        console.error(`fusion: ${event.message}`);
        return;
